...
- Added multilingual translation for X posts: sample mode uses a deterministic local sample dictionary, while local trusted real-provider runs can use the company JoyBuilder Responses API (`JDCLOUD_GPT_API_KEY`, `GPT-5.5`). User-run real smoke test succeeded with one French Joybuy sentence returning Chinese translation. Translation failure now degrades gracefully to original text instead of blocking daily publishing; the dashboard run summary reports translation provider, missing count and original-text fallback count.
